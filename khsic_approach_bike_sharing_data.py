...
#     R = mnn.Parameter(manifold=mnn.Stiefel(d,k-1)).float()
    theta = Variable(torch.randn(d, d).type(dtype), requires_grad=True)
#     R = torch.cat((R1, R2), 1)
...
#             print("Inside the second loop!!!!!!!!!!!!!!!!!!")
            ...
            R = torch.linalg.matrix_exp(torch.triu(theta)-torch.triu(theta).T)
...
#     checkpoint
    ...
    print("(R_mat.T)@R_mat after optimization")
#     print(R)
    print((R_mat.T)@R_mat)
...
#     baseline_ood_accuracy = logistic_regression_on_baseline_og.score(z_test_t, y_test_og)
    ...
    print(f_train[:,ns:].shape)
    lr_model_new_HSIC_no_sp = LinearRegression().fit(f_train[:,ns:],y_train)
    y_pred_h = lr_model_new_HSIC_no_sp.predict(f_test[:,ns:])
    #y_pred_h = lr_model_new_HSIC_no_sp.predict(f_test[:,ns:])
...
if __name__ == "__main__":
    ...
    etas = [1.0]#[0.9,0.95, 0.99, 1.0]#0.99
    batch_sizes = [128]#[64,128,256,512]
    nss = [1,2] #specify number of style features
    epoch_sizes = [50]
